...
async function main() {
  ...
  await orderCheck(playerB, f, (before, after): boolean => {
    ...
    tokenIdx = 1;
    // @ts-ignore
    if (after.player.data.positions[tokenIdx].lock_balance - before.player.data.positions[tokenIdx].lock_balance != 100n*2n) {
      console.log("lock_balance", after.player.data.positions[tokenIdx].lock_balance, before.player.data.positions[tokenIdx].lock_balance);
      return false;
    }
    // @ts-ignore
    if (before.player.data.positions[tokenIdx].balance - after.player.data.positions[tokenIdx].balance != 100n*2n) {
      console.log("balance", before.player.data.positions[tokenIdx].balance, after.player.data.positions[tokenIdx].balance);
      return false;
    }
    return true
