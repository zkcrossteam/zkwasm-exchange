...
CHAIN_ID="56" # Default to Sepolia testnet
...
IMAGE_VALUE="169058E783010001B369EF79692C320F"
...
cat > ${CHART_PATH}/templates/merkle-pvc.yaml << EOL
{{- if and .Values.config.merkle.enabled .Values.config.merkle.persistence.enabled }}
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: {{ include "${CHART_NAME}.fullname" . }}-merkle-rocksdb-pvc
  labels:
    {{- include "${CHART_NAME}.labels" . | nindent 4 }}
  annotations:
    "helm.sh/resource-policy": keep
...
spec:
  ...
  template:
    ...
    spec:
      ...
      - name: rocksdb-data
        persistentVolumeClaim:
          claimName: {{ include "${CHART_NAME}.fullname" . }}-merkle-rocksdb-pvc
{{- end }}
EOL
